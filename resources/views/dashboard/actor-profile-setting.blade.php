...
if ($actor->role == "voice_actor" && $profile->photo_src) {
                        ...
                        <div class="p-4 sm:p-5">
                            ...
                            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                ...
                                <div class="flex sm:col-span-2">
                                    <label class="block">
                                        <span class="text-base font-medium text-slate-600 dark:text-navy-100">
                                            60 Second Recording Cost
                                        </span>
                                        <span class="relative mt-1.5 flex">
                                            <input x-model="formData.price" name="price"
                                            ...
                                            <span
                                                ...
                                                <i class="text-base fa-solid fa-file-invoice-dollar"></i>
                                            </span>
                                        </span>
                                ...
                                <label class="block sm:col-span-2">
                                    ...
                                    <textarea x-model="formData.bio" name="bio" rows="4"
                                        placeholder="Tell about yourself..."
                                        class="form-textarea w-full resize-none rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent mt-1.5"></textarea>

                                    <span class="text-sm text-error" x-text="errors.bio"></span>
                                </label>
                                <label class="block">
                                ...
                                <label class="block">
                                    <span
                                        class="text-base font-medium text-slate-600 dark:text-navy-100">Gender</span>
                                    <span class="relative mt-1.5 flex">
                                        <select x-model="formData.gender" name="gender"
                                    ...
                                    <span class="text-sm text-error" x-text="errors.gender"></span>
                                </label>
                                <div class="grid gap-5 sm:col-span-2 md:grid-cols-2">                                
                                    <label class="block">
                                        <span
                                ...
                                <label class="block">
                                    ...
                                    <span class="relative mt-1.5 flex">
                                        <select x-model.number="formData.deadline" name="deadline"
                                            class="w-full px-3 py-2 text-base bg-transparent border rounded-md form-select form-input peer border-slate-300 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">
                                            <option value="4">4 hours</option>
                                            <option value="12">12 hours</option>
                                            <option value="24">24 hours</option>
                                            <option value="48">48 hours</option>
                                            <option value="48">72 hours</option>
                                        </select>
                                    </span>
                                ...
                                <label class="block">
                                    <span
                                        class="text-base font-medium text-slate-600 dark:text-navy-100">Language</span>
                                    <span class="relative mt-1.5 flex">
                                        <select x-model="formData.language_code" name="language_code"
                                            class="w-full px-3 py-2 text-base bg-transparent border rounded-md form-select form-input peer border-slate-300 pl-9 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">
                                            <option disabled value="">Select your language</option>
                                            @foreach ($languages as $language)
                                            <option {{ $language->code == $profile->language_code ? 'selected' : ''
                                                }} value="{{
                                                $language->code }}">{{ $language->name }}</option>
                                            @endforeach
                                        </select>
                                    </span>

                                    <span class="text-sm text-error" x-text="errors.language_code"></span>
                                </label>
                                <div class="block mb-5 sm:col-span-2">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-base font-medium text-slate-600 dark:text-navy-100">Voice</span>
                                        <span>Every voice is unique, what makes your voice so special?</span>
                                    </div>

                                    <div class="flex flex-wrap gap-2 px-2 mt-4">

                                        @foreach ($voice_specials as $voice_special)
                                        <button type="button"
                                            class="btn min-w-[7rem] rounded-full font-medium transition-all"
                                            @click="toggleSelection({{ $voice_special->id }})"
                                            :class="isSelected({{ $voice_special->id }}) ?
                                        ...
                                        <span class="mt-2 text-sm text-error" x-text="errors.voice_specials"></span>
                                    </div>
                                </div>
    ...
    function profileFormHandler() {
        return {
            ...
            formData: {
                ...
                voice_specials: @json($actor->voiceSpecials->pluck('id')->toArray()), // Extracts IDs,
            ...
            async submitForm() {
                ...
                try {
                    const data = await fetch('{{ route('actorProfileSettings') }}', {
                        method: 'POST',
                        headers: {
